...
pool <- function(dat_combined) {
  ...
  suppress_output(stan('modelBWR.stan', data = data, iter = rstan_iters, chains = n.chains))
}
...
pp <- function(dat_combined, a = 0.5) {
  ...
  suppress_output(stan('modelPP.stan', data = data, iter = rstan_iters, chains = n.chains))
}
...
bdb <- function(dat_combined, nu1 = 0.001, nu2 = 0.001) {
  ...
  data$nu1 <- nu1
  data$nu2 <- nu2
  suppress_output(stan('modelBDB.stan', data = data, iter = rstan_iters, chains = n.chains))
}
