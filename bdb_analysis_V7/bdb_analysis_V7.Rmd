...
```{r}
source("H:/ravi_dsi_summer_project/bdb_analysis_V7/libraries_and_functions_V7.R")
```
...
# This analysis will only cover the sample size of 100. And we will consider only first 5 replicates under each combination.
...
pb <- progress_bar$new(
  ...
  total = length(combined_list), clear = FALSE, width = 60
)

# lapply with progress bar
all_results <- lapply(combined_list, function(df) {
  result <- simulation_func(df)
  pb$tick()
  return(result)
})
```
...
save(all_results, file = "H:/ravi_dsi_summer_project/bdb_analysis_V7/all_results_100_1-5.RData")
```
