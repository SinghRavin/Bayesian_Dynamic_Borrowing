...
# beta_vec = fixed effect parameters 
# rateC = rate parameter of the exponential distribution of C

simulWeib <- function(N, lambda, rho, beta_vec, rateC)
{
  # baseline measured covariates
  ...
  # baseline unmeasured covariates
  U <- rnorm(n=N, mean=0, sd=1)
  
  covariate_mat <- as.matrix(data.frame(age, female, bmi,
                                        liver_mets, hepatic_imp, num_mets_morethan2, U))
  beta_mat <- as.matrix(beta_vec)
  
  # Weibull latent event times
  v <- runif(n=N)
  Tlat <- (-log(v)/(lambda * exp(covariate_mat %*% beta_mat)))^(1/rho)
